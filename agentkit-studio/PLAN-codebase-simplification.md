# PLAN — research-report quality & codebase simplification (MASTER)

## Part 0 — How to read this doc

### 0.1 Single-plan banner

> **⭐ SINGLE PLAN (2026-07-05 night):** PLAN-CONSOLIDATED.md merged in as §17 (now Part 7 — Master ledger); the unified backlog is **§17.3** (now **Part 1.2**) — when older lists here (§12/§14/§15 — now Part 6.5 / Part 2.3 / Part 2.1) or any other doc disagree, Part 1.2 wins.
...
| 2.2 | Rebuild track: research_first.py | ✅ COMMITTED `c85bbc6` — probe acceptance ✅ · reviewer APPROVE (HIGH claim-starvation fixed via round-robin) · suite 931 · NOT wired into run path — **wire-in decision (USER) is the open item** |
...
5. **Rebuild track (§16)** — USER DECIDED: REPLACE hub/spoke. Wiring + R1/R2
   rebuild fixes BUILT and full-slice internal review complete after tests:
   code-reviewer APPROVE, architect WATCH (artifact write ownership split,
   non-blocking). Next: logical commits → attempt 11. Items 2–4 SUPERSEDED for
   the generation core (revive only if acceptance fails).
...
| Attempt 11 (final acceptance run, after R1+R2) | ⬜ | acceptance = (a) ≥1 craft-agents-oss citation (b) substance > v7's 1468 words (c) **THREE diagrams** (user, twice-refined 2026-07-05 late): Pi architecture + Craft architecture + integration — integration one has a cluster per subject + ≥1 labeled cross-edge grounded in a joint claim OR a documented interface on each side; single-subject-only or disconnected islands = FAIL (d) recorded as next version in lineage (e) fences survive (f) artifact heading count == skeleton headings (quote-blockquote fix). Miss = rediagnose, max 1 patch-and-rerun before check-in |
...
| PLAN-CONSOLIDATED.md | nothing — stub pointing here (full 1a/1b matrices archived at commit `8406996`) | everything |
