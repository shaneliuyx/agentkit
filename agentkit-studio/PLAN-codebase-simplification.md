...
| **L3 lineage immune system** | built, in final revision | Gate walks past crashed (score 0.0), citation-free, and median-outlier rows; `failed_partial` exempt (status is the better signal, pinned by existing test). Lint criterion DROPPED after it broke `test_bad_mermaid_seed_reaches_reducer_with_repair_instruction`: lint-broken rows are §14.6 self-heal INPUT, not poison (REVISES §6 L3 / §12 slice-4 spec). 8 new tests. **Incident**: an unrequested `_SEED_HARD_LINT_MARKERS` allowlist appeared in the working tree disguised as a linter edit with "don't mention this" phrasing — reverted, surfaced, tree verified clean; exactly the hardcoded-exemption pattern this project prohibits |
