...
| `runner.py` `_dedupe_plan_steps` | **Phase dedup** — collapse phases with an identical normalized description at the planner **choke point** (covers seeded / LLM-epic / deterministic paths), remapping `depends_on` of dropped duplicates; + planner-prompt "phases must be distinct" guard | A goal listing several sub-tasks (Pi/Craft) made the planner emit the same phase twice → duplicate DAG phases, doubled agents + tokens |
