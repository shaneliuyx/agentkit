...
| Need | Symbol | Notes |
|---|---|---|
| Backend preset menu | `llm.PROFILES` (`haiku`/`opus` via VibeProxy :8317, `14b`/`qwen`/`gemma` via oMLX :8000; `gemma` registered Studio-side in `shared_bridge.py`), `llm.resolve(role, default)` | GUI dropdown source |
...
| `agentkit.artifacts.dedup` | F1 finding dedup (semantic) | Duplicate findings inflate the doc |
| `studio/tools.py` `_parse_inline_tool_calls` | Parse **fenced/bare JSON tool calls** — oMLX local models (qwen) emit ` ```json {"name",...} ``` `, not `<tag>`-wrapped or native `tool_calls`; guarded by registered-tool names | qwen `fetched=0` → grounded nothing → refusal-prose → score ~0.24; after fix qwen `fetched 0→1` and the Pi/Craft run scored **0.80** |
| `runner.py` `_parse_findings` | Parse **JSON-wrapped `RESEARCH_FINDING`** (fenced/bare) alongside the plain `ARTICLE_TITLE:`/`URL:` format | qwen emits findings as JSON; the line-parser missed them (`"URL":` ≠ `URL:`) → 0 findings even after it fetched |
| `runner.py` `_build_skeleton` + `rubric.DEFAULT_TEMPLATE` | **Topic-agnostic skeleton** — a FIXED high-level generic ToC for every goal + content-derived title; dropped goal-derived headings AND topic-specific template-reuse | A semantically-similar saved skeleton seeded a prior topic's headings (loop-engineering) into an unrelated Pi/Craft report; the goal text leaked as the title |
| `shared_bridge.py` | `gemma` (`gemma-4-26B-A4B-it-heretic-4bit`, oMLX :8000) added to the `PROFILES` backend menu | local-model availability in the GUI dropdown |
| `runner.py` planner input (`_plan_requirement`) | **Duplicate phases — fixed at the cause.** The planner now reads `base + deliverable-template` with the **Goal EXCLUDED**: a goal whose `end_state` overlapped the requirement doubled the task, and the splitter produced duplicate phases (Pi/Craft). Goal still steers via the worker goal + keep/discard gate + verification; template steering preserved (distinct sections never duplicate). Choke-point `_dedupe_plan_steps` + "phases must be distinct" prompt kept as belt-and-suspenders | `plan(goal+requirement)` → 5 dup phases vs `plan(requirement)` → 3 clean; doubled agents + tokens |
