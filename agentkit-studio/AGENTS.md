...
# Project Guardrail: Generic Report Generator

AgentKit Studio's report generator is a generic research report generator.
This constrains Studio source code and built-in mechanisms, not the generated
report content. Generated reports must be specific to the user's task, topic,
audience, and evidence. Production code must not hardcode topic-specific report
prose, example conclusions, model-specific report branches, or fixed templates
for one scenario. Domain-specific content belongs only in user input, tests,
fixtures, reference materials, fetched evidence, or reviewed catalog/template
data. Prefer LLM-driven planning, generic evidence contracts, publish gates, and
reusable prompts.

# Project Guardrail: Validation Drift

Each completed test or E2E verification must be compared with the previous
comparable run before it is treated as success. Report whether quality,
coverage, topology, generated artifacts, logs, scores, or failure modes drifted.
Unexpected drift is a validation failure to investigate, even when the latest
test command exits successfully. When the previous comparable run failed, the
current run must show measurable improvement against that failure before it
counts as success.

# Project Guardrail: Section Assignment Queue

Report section workers must fetch one section file per worker call from the
assignment queue. Do not batch multiple section files into one worker output.
If there are more section files than concurrent workers, keep queueing one-file
worker foci until all active section files are assigned. Delete an assignment
row only after the worker call for that row has completed. Each queue row must
include the full worker assignment text, not only ids, so a worker can execute
from the row alone.
