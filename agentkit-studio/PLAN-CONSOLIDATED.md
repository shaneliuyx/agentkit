...
| **Sections — H3 sub-structure** | 🟡 IN PROGRESS (upstream, task 3/3) | `_write_section` WRITE-prompt directive to emit `###` sub-topics. Non-deterministic (model-dependent) but self-contained — no schema/extraction touch. Sequenced LAST (cheapest, lowest-risk of the trio). |
| **Detail — provider-routing table** | 🟡 IN PROGRESS (upstream, task 1/3) | **Re-derived against code (2026-07-09 PM):** routing facts (Pi SDK→Google/Codex/OpenAI, Claude Agent SDK→Anthropic) ARE present as prose in fetched sources — a *structure* gap, not coverage. Adding a `relations:[{head,rel,tail}]` field to the claim schema at extraction (reuses the existing KGGen triple pattern) lets ASSEMBLE group them into a table with zero hardcoded lexicon. Highest value×tractability → sequenced FIRST. |
| **Detail — dep-order + pi-tui** | ⬜ BLOCKED UPSTREAM (coverage, task 2/3) | **Re-derived:** `pi-tui` in **0/40** claims is a *coverage* miss (the model never extracted it), NOT a structure gap — a `relations` field can't emit a name absent from evidence. Needs an extraction-*coverage* fix (capture the full package roster) BEFORE any dependency-edge structuring. Hardest + least deterministic → sequenced SECOND, after provider-relations proves the schema extension. |
