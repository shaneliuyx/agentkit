...
- ✅ **L5 DONE** (`1589a03`) — acceptance economics: `build_pass_economics` (per-pass attempted/accepted + tokens-per-accepted) wired into `build_run_metrics`; verdict-memoization half was already ✅.
...
| E8 | Executive summary consistency: no claim in the summary that the body doesn't support | LLM | ✅ BUILT deterministic 2026-07-12 (`_e8_summary_overclaim`: summary-only citation markers = overclaim; LLM half skipped per doctrine) |
| E9 | Internal consistency: entity names/numbers stable across sections | LLM | NEW — one judge call (cheap, over stripped text) |
| E10 | Honest limitations: Limitations names the ACTUAL gaps (ledger not-found rows, thin-source subjects), not boilerplate | DET+LLM | ✅ BUILT deterministic 2026-07-12 (`_e10_undisclosed_gaps`: coverage-ledger gap not named in Limitations = fail; LLM half skipped per doctrine) |
...
| 8 | E7 | dyn-section birth certificate + requirement-conditioned check | ✅ DONE `cb5897a` (deterministic advisory row; LLM half skipped per doctrine) |
| 9 | L4 | repeat-weakness escalation (uses S2 pass ledger + L1 row evidence) | ✅ MEASURED-NULL for research_first (cold-start, no epoch loop to escalate across — see Part 1.1); applies to legacy fallback only |
| 10 | D1 | claims-with-provenance layer | major slice |
| 11 | D4 + E8/E9 | write-order DAG (summary-last) + consistency judges | ✅ DONE — D4 realized-by-construction (`_write_summary` last); E9 `cb5897a`, E8 `<this commit>` (both deterministic advisory). D4 prevents, E8 detects |
| 12 | P2 | question-first planning | after L1 changes what verify checks |
| 13 | L5+S5 | economics: verdict memoization (done); rubric memo + cache index MEASURED NULL 2026-07-10 (S5 ✅ no code); per-run cost line (L5) | ✅ DONE — S5 measured-null (no code); L5 `1589a03` (`build_pass_economics`: per-pass attempted/accepted + tokens-per-accepted, wired into `build_run_metrics`) |
...
> **Status note (2026-07-08):** the addendum's four **Required Recovery Changes** are now largely DELIVERED — (2) generic N-subject relationship model = MVP-8 R3 (`_classify_relationship` {cooperates/competes/extends/alternative/independent/unknown}, claim-grounded, enum removed); (3) evidence-bearing diagram labels = MVP-5b (proper-component naming, glue-word drop); (4) markers publish-blocking = done (`evaluate_publish_readiness` rejects `RESEARCH_FINDING` etc.). (1) fail-visible per-subject coverage gate before WRITE is ✅ REALIZED (2026-07-12, code-verified) — by a SUPERIOR mechanism to the addendum's literal hard-stop: `_coverage_gate` fires one grounded recovery fetch for a thin/zero-source subject, then on still-thin emits a fail-VISIBLE `coverage_failed_partial` SSE event and proceeds; `_recover_underevidenced` (P2.5) later converts a still-short subject into a declared question-limitation (never weak fallback prose), with `_classify_answerability` routing structurally-unanswerable asks to reasoned Limitations, backstopped by E4 (ungrounded-text drop) + E7 (empty-dynamic-section) + E10 (undisclosed-gap) detectors. A hard stop is a SETTLED NEGATIVE — it would abort the whole report because one subject is thin, discarding good coverage on every other; graceful-partial + honest-limitation degrades one section, matching the plan-wide "honest Limitation over fabrication" rule. The reference material below is kept for future provenance decisions.

### 8.1 Required Recovery Changes (original addendum text)

1. **Fail-visible per-subject coverage gate before WRITE.** Each requested subject must have ≥1 accepted source and ≥1 claim-bearing evidence path before section writing. One generic targeted recovery query allowed for a zero-source subject; if still empty, stop with `failed_partial`, not weak fallback prose. *(✅ REALIZED as graceful-partial + honest-limitation, not literal hard-stop — see status note above.)*
