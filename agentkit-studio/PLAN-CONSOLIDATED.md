...
| G4 | **Citation stuffing** — same URL repeated after every sentence | `pi.dev/docs/latest` ×N | **NEW — citation diversity/dedup**: distinct source per claim; cap repeats of one URL | ◐ MOSTLY — raw-URL stuffing eliminated by References-B `[N]` render + adjacent-dup collapse (`4fb0240`); per-paragraph repeat cap = in the running workflow |
...
| **References (G3/G4)** | ✅ A+B DONE | **A (titled numbered entries): ✅ `030ffba`** live-verified — `N. [page-title](url)`. **B (inline `[N]` citations): ✅ `4fb0240`** live-verified (v82/v83, 32 markers) — deterministic `_apply_citation_markers` renders each inline URL the writer already cited → its reference number `[N]` (better than codex's writer-proposes C: the writer emits explicit URLs, so the marker is a *render* of a declared citation, zero false-attribution; also de-stuffs G4 raw-URL clutter). **Grounding residual: ✅ `5009d8a`** — a same-domain non-claim URL (hallucinated `pi.dev/packages/pi-agent-workflows`) is now STRIPPED (fake citation removed, domain-grounded sentence kept), never a guessed marker. G4 per-paragraph cap = in the running workflow. |
...
| **Sections (G6 heading leaks)** | ✅ DONE `5009d8a` | `_demote_stray_h1` artifact-wide backstop: exactly one H1 (title), a leaked body `# ` demotes to `### `, fence-aware. Live v83: 1 H1. Complements per-section `_sanitize_section_headings`. H3 sub-structure (flatness, distinct from leaks) = in the running workflow. |
| **Detail** | ◐ IN WORKFLOW | ~80%; dependency order + pi-tui + provider-routing table = tasks in the running Part-1 workflow (`finish-part1-gold-parity`). |
