...
| **Detail — dep-order + pi-tui** | 🟡 PARTIALLY CRACKED via relations | **Discovery from live relations:** `Pi includes TUI` (→ pi-tui surfaces as "TUI") and `pi-agent-core sits between pi-ai and pi-coding-agent` (→ dependency order) both arrive FREE through the `relations` field — the "coverage-only" framing was too pessimistic; structured relations partly solve it. Remaining gap: the literal token `pi-tui` still under-appears vs the gold roster. A dedicated roster-completeness extraction pass is the residual, now LOWER priority since the dependency/roster story lands in the relationship table. |
