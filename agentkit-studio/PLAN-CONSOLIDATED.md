...
| **References (G3/G4)** | ◐ MOSTLY DONE | **A (titled numbered entries): ✅ `030ffba`**, live-verified — `N. [page-title](url)`. **B (inline `[N]` citations): DONE on disk, live-verify pending** — deterministic `_apply_citation_markers` maps each inline URL the writer already cited → its reference number `[N]` (better than codex's writer-proposes approach C: the writer emits explicit URLs, so the marker is a *render* of a declared citation, zero false-attribution risk; also de-stuffs G4 raw-URL clutter). Residual: same-domain non-claim URLs (e.g. a hallucinated `pi.dev/packages/*`) that domain-grounding permits are left raw (not mapped to a wrong marker) — a grounding-strictness gap, not a B bug. |
