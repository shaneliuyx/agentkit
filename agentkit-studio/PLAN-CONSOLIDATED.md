...
| 5.2 | Editorial gate E1–E11 | ◐ L1 unifier LANDED `371861f` — **7/11 DET rows compute (E1–E6, E11)**; fail-open never records a pass (reviewer HIGH + codex HIGH×2 fixed); verdict router + seed-eligibility gated. **NOT emitted: the 4 LLM-judge rows E7/E8/E9/E10** (verified `grep '_row("E..")'` in finalize.py = E1-E6+E11 only). Failure modes mostly PREVENTED by construction (E8←D4 summary-from-body, E10←P4 `_limitations_note`, E7←structural-producer grounding gates) but the DETECTOR rows don't run; **E9 (entity/number stability) has no coverage at all** — the one genuine detection gap. |
