...
| **Sections — H3 sub-structure** | ⬜ DEFERRED (needs prompt change) | Workflow `finish-part1-gold-parity` design: feasible but NON-deterministic — requires a `_write_section` WRITE-prompt directive to emit `###` sub-topics (not a deterministic post-pass). Kept out of the auto-implement lane; a separate careful prompt pass. |
| **Detail** | ⬜ BLOCKED UPSTREAM (extraction/schema) | Workflow design proved both halves un-doable at ASSEMBLE without hardcoding: **dep-order + pi-tui** — `pi-tui` is named in **0/40** Pi claims (roster claim says "three Pi packages"); a claims-only pass can't emit a name absent from evidence → needs a RESEARCH/extraction fix (capture package rosters + dependency edges as claim fields). **provider-routing table** — claim schema `{claim,quote,url,subjects}` has no structured routing relation; any deterministic parser needs a hardcoded provider/verb lexicon → needs a claim-schema `relations:[{head,rel,tail}]` field at extraction time. Both are NEXT-LAYER (RESEARCH/CLAIMS stage), not ASSEMBLE. |
