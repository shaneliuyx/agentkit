...
export const FIXTURE_EVENTS: StudioEvent[] = [
  ...
  // ── M7 Wave 1: loop catalog match + pre-seed ──
  {
    type: "loops",
    session_id: SID,
    ts: t(),
    payload: {
      matches: [
        {
          id: "loop-research-synthesis",
          title: "Research → Synthesize → Verify",
          summary: "Gather sources, cross-examine in a debate, refine, then verify claims.",
          url: "https://catalog.example/loops/research-synthesis",
          trigger: "research",
          keywords: ["research", "synthesize", "verify", "debate"],
          score: 0.92,
        },
        {
          id: "loop-quick-lookup",
          title: "Quick Lookup",
          summary: "Single-agent fast factual lookup with one web search.",
          url: "https://catalog.example/loops/quick-lookup",
          trigger: "lookup",
          keywords: ["lookup", "fast", "factual"],
          score: 0.54,
        },
      ],
    },
  },
  {
    ...
    ts: t(),
    payload: {
      loop_id: "loop-research-synthesis",
      steps: [
        { id: "s1", description: "Gather sources", depends_on: [], role: "researcher" },
        { id: "s2", description: "Cross-examine claims", depends_on: ["s1"], role: "reviewer" },
      ],
    },
  },
  {
