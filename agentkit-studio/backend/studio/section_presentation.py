...
def _detect_prompt(heading: str, body: str) -> str:
    ...
    return (
        ...
        "3 or more distinct named components/steps connected by REAL structure between "
        "them: either flow/calls (A calls B, X feeds Y, a pipeline, an architecture) OR "
        "composition/hierarchy/layering (A contains B, X belongs to the Y layer, parts "
        "grouped into a system, one tier built on another)?\n"
        "A narrative, an executive summary, a list of separate findings, or a citations "
        "section is PROSE, never a diagram.\n"
