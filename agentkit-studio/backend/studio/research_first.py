...
#: A research question longer than this many words is likely a restated task
#: paragraph, not a crisp question — dropped so queries stay focused (P2).
...
def _subject_queries(
    ...
    *,
    ...
    """The per-subject search queries (extracted for unit-testing). With P2 questions
    present: one ``{descriptor} {question}`` query per question so search targets what
    the requirement ASKS, plus a bare-anchor fallback so the subject still gathers if
    its question queries return thin. Without questions: the original descriptor/anchor
    triple (byte-identical). Every query carries the descriptor, so the downstream
    anchor/name CONTENT gates are unaffected."""
    qs = [q for q in (questions or []) if q and q.strip()]
    if qs:
        out = [f"{descriptor} {q}".strip() for q in qs[: max(1, cap - 1)]]
        out.append(f"{descriptor} {anchor_phrase}".strip())  # fallback: still gather
        return out
...
def _research(
    ...
    *,
    ...
    for subject in subjects:
        ...
        # descriptor+anchor query rank the actual product (team-lead fix 2). P2: with
        # per-subject questions, search for their ANSWERS (descriptor+question) +
        # a bare-anchor fallback; without questions, the original triple (unchanged).
        queries = _subject_queries(
            descriptor, subject, anchor_phrase, (questions or {}).get(subject),
            cap=_MAX_QUERIES_PER_SUBJECT,
        )
...
def generate_research_first(
    ...
    *,
    ...
    ledger, assumptions, relationship, all_anchors, coverage = _research(
        subjects, evidence_dir, requirement, judge_client, questions=questions, emit=emit
    )
