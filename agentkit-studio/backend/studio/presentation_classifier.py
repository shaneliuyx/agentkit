...
def _adjudicate_prompt(heading: str, body: str) -> str:
    ...
    return (
        ...
        "- TABLE: 2 or more named entities compared or contrasted across 2 or more shared "
        "attributes/dimensions — count this EVEN when the comparison is written as prose "
        "sentences, not only when the section already holds a table (e.g. 'X is faster but "
        "pricier, whereas Y is slower but cheaper' compares X and Y across speed and price "
        "-> TABLE). Also a list of >8 items. When a section BOTH names connected parts AND "
        "weighs entities across attributes, prefer TABLE if the point is how they COMPARE.\n"
